...
sub = dir('first_level_subjcon*');                    %copy the 1st level directory
load VAS_HV.mat;                                    %load your interpolation file
...
if VAS_HV{jl}(a+49+(i-1)*147) == 0;
    matlabbatch{1}.spm.stats.con.consess{a+(jl-1)*147}.tcon.weights = [zeros(1, a-1) VAS_HV{jl}(a+(i-1)*147) zeros(1,48) 0.00000000000001];   %0.0001 cause you cannot create contrasts with all zero's
else
matlabbatch{1}.spm.stats.con.consess{a+(jl-1)*147}.tcon.weights = [zeros(1, a-1) VAS_HV{jl}(a+(i-1)*147) zeros(1,48) -VAS_HV{jl}(a+49+(i-1)*147)];
end;
matlabbatch{1}.spm.stats.con.consess{a+(jl-1)*147}.tcon.sessrep = 'none';
 ...
 if VAS_HV{jl}(b+98+(i-1)*147) == 0;
    matlabbatch{1}.spm.stats.con.consess{d+(jl-1)*147}.tcon.weights = [zeros(1, b-1) VAS_HV{jl}(b+(i-1)*147) zeros(1,97) 0.00000000000001];   %0.0001 cause you cannot create contrasts with all zero's 
else
matlabbatch{1}.spm.stats.con.consess{d+(jl-1)*147}.tcon.weights = [zeros(1, b-1) VAS_HV{jl}(b+(i-1)*147) zeros(1,97) -VAS_HV{jl}(b+98+(i-1)*147)];
end;      
...
if VAS_HV{jl}(c+98+(i-1)*147) == 0;
    matlabbatch{1}.spm.stats.con.consess{e+(jl-1)*147}.tcon.weights = [zeros(1, 49) zeros(1, c-1) VAS_HV{jl}(c+49+(i-1)*147) zeros(1,48) 0.00000000000001];   %0.0001 cause you cannot create contrasts with all zero's
else
matlabbatch{1}.spm.stats.con.consess{e+(jl-1)*147}.tcon.weights = [zeros(1, 49) zeros(1, c-1) VAS_HV{jl}(c+49+(i-1)*147) zeros(1,48) -VAS_HV{jl}(c+98+(i-1)*147)];
end;      
...
eval(['save Batch_interpolate_VAS_HV_' sub(i).name(13:22) '.mat matlabbatch']);    %save batch job and rename
cd(root);
spm_jobman('initcfg')
